...
//		return null as no path exists
		Queue<WPTreeNode> exploreQueue = new LinkedList<>();
		...
		while(!exploreQueue.isEmpty()) {	
			WPTreeNode current = exploreQueue.remove();
			for(String node: nw.distanceOne(current.getWord(), true)) {
				if(!visited.contains(node)) {
					visited.add(node);
					current.addChild(node);
					
					if(node.equals(word2)) {
						return new WPTreeNode(word2, current).buildPathToRoot();
					}
				}
			}
			exploreQueue.addAll(current.getChildren());
		}
