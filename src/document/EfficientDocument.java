...
public class EfficientDocument extends Document {
     ...
     */
	private int words = 0, sentences = 0, syllables = 0;
	private void processText() {
		...
		for(String token: tokens) {
			if(isWord(token)) {
				...
				//if we reach the last word in the list, that counts as another sentence
				if(tokens.get(tokens.size()-1).equals(token)) sentences++;
			} else sentences++;
		}
...
//		EfficientDocument ed = new EfficientDocument("This is a test.  How many???  "
