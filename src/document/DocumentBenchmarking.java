...
public class DocumentBenchmarking {	
	public static void main(String [] args) {
		...
		System.out.println("Num of chars \t Basic Time \t Efficient Time");
		for (int numToCheck = start; numToCheck < numSteps*increment + start; numToCheck += increment) {
			...
			for(int i=0; i<trials; i++) {
				bd = new BasicDocument(characters);
				bd.getFleschScore();
			}
			long endTimeBd = System.nanoTime();
			System.out.print((endTimeBd - startTimeBd) + "\t" + " ");
			
			
			EfficientDocument ed;
			long startTimeEd = System.nanoTime();
			for(int i=0; i<trials; i++) {
				ed = new EfficientDocument(characters);
				ed.getFleschScore();
			}
			long endTimeEd = System.nanoTime();
			System.out.println((endTimeEd - startTimeEd));
